...
// packages/react-keybound/src/vite.ts
function keybound(options = {}) {
  const metadata = /* @__PURE__ */ new Map();
  const isSource = (filename) => /\.[cm]?[jt]sx$/i.test(filename) && !filename.includes("\0") && !/(?:^|[\\/])node_modules(?:[\\/]|$)/.test(filename);
  return {
    name: "react-keybound",
    enforce: "pre",
    buildStart() {
    ...
    generateBundle() {
      ...
      this.emitFile({
        type: "asset",
        fileName: typeof options.manifest === "string" ? options.manifest : "keybound-manifest.json",
